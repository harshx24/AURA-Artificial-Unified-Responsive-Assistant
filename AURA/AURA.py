...
def ASK():
    ...
    bot_response = action.handle_user_input(user_val)
    user_val = entry.get() or ""  
    text.insert(END, "You: " + str(user_val) + "\n")

    if bot_response != None:
        text.insert(END, "BOT: " + str(bot_response) + "\n")
    if bot_response == "shutting down, goodbye" :
        root.destroy()

#send function        
def SEND():
    send = entry.get()
    bot = action.handle_user_input(send)
    text.insert(END,"You: "+ send+"\n")
    if bot != None:
         text.insert(END, "BOT: " + str(bot) + "\n")
    if bot == "Shutting down, Goodbye" :
         root.destroy()
...
# Button function
Button1 = Button(root, text="  ASK  ", font=("Arial", 14), bg="#393B39", pady= 20, padx =20, borderwidth= 2,relief = SOLID,command=ASK, fg="white")
Button1.place(x=220, y=620)

Button2 = Button(root, text=" SEND ", font=("Arial", 14), bg="#393B39", pady= 20, padx =20, borderwidth= 2,relief = SOLID,command=SEND, fg="white")
Button2.place(x=348, y=620)

Button3 = Button(root, text="DELETE", font=("Arial", 14), bg="#393B39", pady= 20, padx =20, borderwidth= 2,relief = SOLID,command=DELETE, fg="white")
